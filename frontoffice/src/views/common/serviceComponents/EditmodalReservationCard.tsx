...
const EditmodalReservationCard = (props: {
  showModal: boolean
  setShowModal: any
  openService: string
  animalReservation: JsonReservation.IReservation[]
}) => {
  ...
  return (
    <>
      <div
        data-aos="fade-in"
        className="py-12 flex   transition duration-150 ease-in-out z-10 absolute top-20 right-0 bottom-0 left-0"
        ref={openRef!}
      >
        <div data-aos="zoom-in" role="alert" className="container mx-auto w-11/12 md:w-2/3 max-w-lg">
          ...
          >
            ...
            <ToastContainer />
            <label htmlFor="selectService" className="text-gray-800 text-sm font-bold leading-tight tracking-normal">
              Select a Service
            </label>
            <select
              onChange={(value) => {
            ...
            {infoReservation && (
              <div data-aos="zoom-in">
                <label htmlFor="information" className="text-gray-800 text-sm font-bold leading-tight tracking-normal">
                  More Information
                </label>
                <div className="relative mb-5 mt-2">
                  ...
                  <textarea
                    rows={2}
                    maxLength={100}
                    onChange={(text) => setInformation(text)}
                    defaultValue={infoReservation.information}
                    className="text-gray-600 focus:outline-none focus:border focus:border-indigo-700 font-normal w-full flex items-center pl-16 text-sm border-gray-300 rounded border resize-none"
                  />
                </div>
                <label htmlFor="data" className="text-gray-800 text-sm font-bold leading-tight tracking-normal">
                  Date
                </label>
                <div className="relative mb-5 mt-2">
                  ...
                  <input
                    ...
                    defaultValue={infoReservation.date}
                    type="datetime-local"
                    min={new Date().toISOString().slice(0, 16)}
                    max={'2024-01-00T10:00'}
                    className="text-gray-600 focus:outline-none focus:border focus:border-indigo-700 font-normal w-full h-10 flex items-center pl-3 text-sm border-gray-300 rounded border"
                    required
                  />
                </div>
                <label htmlFor="location" className="text-gray-800 text-sm font-bold leading-tight tracking-normal">
                  Select a Location
                </label>
                <div className="relative mb-5 mt-2">
                  <div className="absolute right-0 text-gray-600 flex items-center pr-3 h-full cursor-pointer"></div>
                  <select
                    onChange={(value) => setLocationSelect(value.target.value)}
                    defaultValue={infoReservation.locationId}
                    className="mb-5 mt-2 text-gray-600 focus:outline-none focus:border focus:border-indigo-700 font-normal w-full h-10 flex items-center pl-3 text-sm border-gray-300 rounded border"
                  >
                ...
                <div className="flex items-center justify-start w-full">
                  <button
                    onClick={async () => await editReservation()}

...
const goToVideoWall = () => router.redirect(router.FunnyVideosRoute)
...
onBeforeMount(() => {
  ...
  for (let i = 0; i < VIDEO_NO; i++) randomAnimalTypes.value.push(calculateRandomAnimalType())
})
</script>
<template>
  <div v-if="randomAnimalTypes" class="p-5 animate-in fade-in zoom-in duration-500">
    <h2 class="text-text font-black text-6xl text-center my-5">Videos</h2>
    <div class="flex justify-center flex-wrap">
      <VideoCard v-for="atype in randomAnimalTypes" :key="atype" :animal="atype" />
    </div>

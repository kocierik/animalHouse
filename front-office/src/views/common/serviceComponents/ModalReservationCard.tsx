...
const ModalReservationCard = (props: { showModal: boolean; setShowModal: any; openService: string }) => {
  ...
  const postReservation = async () => {
    if (Helpers.getUserId()) {
      if (
        date?.target.checkValidity() &&
        locationSelect &&
        locationSelect.target.value != 'Select...' &&
        animalSelect &&
        animalSelect.target.value != 'Select...' &&
        date
      ) {
        const reservation: JsonReservation.IReservation = {
          animalId: animalSelect.target.value,
          serviceId: props.openService,
      ...
      >
        <div data-aos="zoom-in" role="alert" className="container mx-auto w-11/12 md:w-2/3 max-w-lg">
          ...
          >
            ...
            <h1 className="text-gray-800 font-lg font-bold tracking-normal leading-tight mb-4">{serviceName}</h1>
            <ToastContainer />
            {
              <>
                ...
                <select
                  onChange={(value) => setAnimalSelect(value)}
            ...
            <div className="flex items-center justify-start w-full">
              <button
                onClick={async () => await postReservation()}

...
import { UserReview } from '../../Product';

const Reviewer = (props: {review : UserReview }) => {
        ...
        <>
          <div className="pt-3 pb-3 md:pb-1 px-4 md:px-16 bg-white bg-opacity-40">
            <div className="flex flex-wrap items-center">
              ...
              <h4 className="w-full md:w-auto text-xl font-heading font-medium">{props.review.name}</h4>
              <div className="w-full md:w-px h-2 md:h-8 mx-8 bg-transparent md:bg-gray-200"></div>
              <span className="mr-4 text-xl font-heading font-medium">{props.review.star}</span>
              <div className="inline-flex">
                {Array.from(Array(props.review.star), (e, i) => {
                  return (
          ...
          <div className="px-4 overflow-hidden md:px-16 pt-8 pb-12 bg-white">
            <div className="flex flex-wrap">
              <div className="w-full md:w-2/3 mb-6 md:mb-0">
                <p className="mb-8 max-w-2xl text-darkBlueGray-400 leading-loose">{props.review.description}</p>
              </div>
              <div className="w-full md:w-1/3 text-right">
                <p className="mb-8 text-sm text-gray-300">{props.review.data}</p>
              </div>
            </div>
          </div>
        </>

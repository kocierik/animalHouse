...
export const getAnimalCodes = async (_: Request, res: Response) =>
  res.status(STATUS_OK).json((await AnimalCode.find({})).map((x) => ({ code: x.code, value: x.value })))

export const getAnimal = async (req: Request, res: Response) => {
  const animal = await Animal.findOne({ _id: req.params.id })

  if (!animal) return res.status(STATUS_BAD_REQUEST).json(new JsonError(`Can\'t find animal with id ${req.params.id} `))

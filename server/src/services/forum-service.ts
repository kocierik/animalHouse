...
export const checkPostAccess = async (userId: string, postId: string) => 
    userId === (await Post.findById(postId)).userId ||
...
export const getPostOfForum = async (forumId: string) : Promise<JsonPost[]> => {
    if(await Forum.exists({_id: forumId}))
        return (await Post.find({forumId: forumId})) as JsonPost[]
